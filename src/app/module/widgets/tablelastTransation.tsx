      ...
      case "userAdress":
        return (
          <div className="flex flex-col">
            <p className="text-bold text-sm capitalize text-teal-400">+1 {datos.userAdress}</p>
         {/*    <p className="text-bold text-sm capitalize text-default-400">{datos.address}</p> */}
          </div>
        );
      case "orderStatus":
        return (
          <Chip className="capitalize" color={statusColorMap[datos.orderStatus]} size="sm" variant="flat">
            {datos.orderStatus }
          </Chip>
        );
        case "amount":
          return (
            <p>$ {Math.floor(datos.amount)}</p>
          )
      default:
        return cellValue;
    }
  ...
  return (
    <Table 
     aria-label="Example table with custom cells" >
      ...
      <TableBody className="bg-gradient-to-br from-rose-400 to-purple-400" items={data || []}>
        {(item:any) => (
