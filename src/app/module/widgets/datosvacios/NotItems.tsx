  ...
  return (
    <div onMouseLeave={handleEffectHoverL} onMouseEnter={handleEffectHoverE} className='w-full relative bg-gradient-to-tr from-violet-50 border via-rose-50 rounded-xl h-96 flex flex-col justify-center items-center'>
       <div className='border-2  border-dashed rounded-lg flex flex-col justify-center items-center mx-20 py-7'>
        ...
        <FaStore className={`bg-gradient-to-tr text-slate-400 text-9xl ${effect && 'animate-pulse'}`}/>
        <div
        ...
        <p className="text-2xl w-1/2 text-center font-bold bg-gradient-to-tr from-slate-400 bg-clip-text text-transparent to-rose-200">
          Don't data that show. First you have that add new Store . Follou next link for start  
            </p>

            <button  className={` mt-7 text-white bg-gradient-to-br from-purple-600 to-blue-500 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2 hover:animate-none ${effect && 'bg-gradient-to-bl ring-4 outline-none ring-blue-300 animate-bounce'}`}><Link to="/stores/add">Click here</Link></button> 
          </div>
    </div>
  )
}
