...
export const CustomerReviews = ({
  ...
  const {
    ...
    url: `reviewLike/like/${usuarioId}?refresh=${refresh}`,
  });
...
console.log(!loadingData)
  ...
  const ReviewItem = React.memo(({ item }: { item: any }) => {
    ...
    const sendlikeorNot = async (
      reviewId: number,
      type: "like" | "dontLike"
    ) => {
      let respuesta: any;
      seticonloading({ iconloading: true, type: type });
      switch (type) {
        case "like":
          try {
            respuesta = await axios.post(port + `reviewLike/like/`, {
              usuarioId,
              reviewId,
              type,
            });
      ...
      const newDatos = mergedData.map((item) =>
        item.id === reviewId
          ? {
              ...item,
              likes: respuesta.data.updatedReview.likes,
              dontlikes: respuesta.data.updatedReview.dontlikes,
            }
          : item
      );
      setrefresh(refresh + 1);
      setMergedData(newDatos);
      seticonloading({ iconloading: false, type: type });
    };
    const conditionlike = like?.some(
      (e: any) => e?.reviewId == item.id && e.likeorNot === "like"
    );

    const conditionDontLike = like?.some(
      (e: any) => e?.reviewId == item.id && e.likeorNot === "dontLike"
    );
      ...
      <div className="flex flex-col md:flex-row justify-between w-[90%] m-5">
        ...
        <div className="flex flex-col gap-2 pl-3 w-[95%] md:w-[70%]">
          ...
          <div className="flex mt-2 gap-2">
            ...
            >
              {((iconloading.iconloading && iconloading.type === "like") || loadingData)  ? (
                <Spinner size="sm" labelColor="danger" color="danger" />
              ) : conditionlike ? (
            ...
            <div
              ...
              className="flex items-center gap-2 cursor-pointer"
            >
              {((iconloading.iconloading && iconloading.type === "dontLike") || loadingData) ? (
                <Spinner size="sm" labelColor="danger" color="danger" />
              ) : conditionDontLike ? (
