  ...
  return (
    <div onMouseLeave={()=> setisEnter(false)} onMouseEnter={()=> setisEnter(true)} className='w-full flex justify-center items-center h-full relative'>
        ...
        <div className='z-20 text-white  w-full pl-5 h-full justify-center items-start flex flex-col '>
            ...
            <button onClick={()=> redirect('/stores/edit')} className={`${isEnter && "animate-pulse "} z-10 mt-4 hover:scale-95 text-white bg-gradient-to-br from-orange-400 to-rose-600 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-blue-300font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2`}>
            Delite +Stores
          </button>
        </div>
    </div>
